...
type Props = { searchParams: Promise<{ status?: string; selected?: string; property?: string }> }

export default async function MarketingContentPage({ searchParams }: Props) {
  await requireMarketingAdminPage()
  const { status, selected, property } = await searchParams
