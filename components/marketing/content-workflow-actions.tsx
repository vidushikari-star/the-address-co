...
export function ContentWorkflowActions({
  ...
  function render() {
    ...
    startTransition(async () => {
      ...
      setMessage(response.ok ? "Creative rendering has been queued." : data.error ?? "Could not queue the creative render.")
  ...
  if (content.status === "failed" && requiresRender) {
    ...
    if (!mayBeAmbiguous && hasReadyMedia) return <section className="space-y-3 rounded-xl border border-red-200 p-4"><p className="text-sm font-semibold">Publishing failed</p><p className="text-xs text-muted-foreground">{publication?.lastError ?? content.lastError ?? "The Instagram publication did not complete."}</p><div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant="outline" onClick={returnToApproved} disabled={isPending}><ArrowLeft className="h-4 w-4" />Return to Approved</Button>{publishingEnabled && <Button type="button" size="sm" variant="outline" onClick={retryPublishing} disabled={isPending}><RotateCcw className="h-4 w-4" />Retry Publishing</Button>}</div>{message && <p className="text-xs text-muted-foreground" role="status">{message}</p>}</section>
    return <section className="space-y-2 rounded-xl border border-red-200 p-4"><p className="text-sm font-semibold">Render failed</p><p className="text-xs text-muted-foreground">{content.lastError ?? "Fix the format-ready creative, then re-approve before retrying. You cannot schedule or publish a failed render."}</p><Button type="button" size="sm" variant="outline" onClick={render} disabled={isPending}><Clapperboard className="h-4 w-4" />Retry render</Button>{message && <p className="text-xs text-muted-foreground" role="status">{message}</p>}</section>
  }
