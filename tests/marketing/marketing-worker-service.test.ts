...
describe("MarketingWorkerService render queue", () => {
  ...
  it("does not requeue a generation that exhausted its one token-limit recovery", async () => {
    ...
    const process = vi.spyOn(privateWorker(), "process").mockRejectedValue(new ContentGenerationTooLongError())
    let jobTableCalls = 0
    admin.client.from.mockImplementation((table: string) => {
