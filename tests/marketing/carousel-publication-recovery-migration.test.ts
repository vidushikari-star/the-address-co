...
describe("Carousel publication recovery migration", () => {
  ...
  it("keeps ambiguous media_publish attempts out of automatic recovery", async () => {
    ...
    expect(migration).toContain("reconciled_orphans integer := 0;")
    expect(migration).toContain("with recovered_orphans as (")
    expect(migration).toContain("), repaired_orphans as (")
    expect(migration).toContain("), ambiguous_publications as (")
    expect(migration).toContain("), failed_orphans as (")
    expect(migration).toContain("select count(*) into reconciled_orphans\n  from failed_orphans;")
    expect(migration).not.toContain("perform 1 from failed_orphans;")
    expect(migration).not.toContain("leaseexpired")
  })
})
