...
const repository = vi.hoisted(() => ({ getContentById: vi.fn(), getReelVersion: vi.fn(), queueReelVersionRender: vi.fn(), addAuditLog: vi.fn() }))
...
beforeEach(() => {
  ...
  repository.queueReelVersionRender.mockResolvedValue({ id: "job-1", type: "render_reel", status: "queued" })
  repository.addAuditLog.mockResolvedValue(undefined)
})

describe("POST /api/marketing/content/:id/reel/versions/:versionId/render", () => {
  it("queues the revised version with its parent state transition atomically", async () => {
    ...
    expect(repository.queueReelVersionRender).toHaveBeenCalledWith(expect.objectContaining({
      contentId,
      versionId,
      updatedBy: "admin-1",
    }))
  ...
  it("requires approval before the existing queue may render a revised version", async () => {
    ...
    expect(repository.queueReelVersionRender).not.toHaveBeenCalled()
  })
})
